...
This work was conducted before the release of \textit{Gaia} DR2 \citep{Gaia_collaboration_2018} that contains parallaxes based only on \textit{Gaia} and are no longer reliant on the Tycho-2 catalogue. Here, a comparison between the distances used in this analysis and the distances from \textit{Gaia} DR2 is presented. Parallaxes for 22 out of 24 stars were found in the \textit{Gaia} DR2 archive; no comparison for the Sun is required and Alpha Cen B was not found in the \textit{Gaia} DR2 archive. The distances used in this analysis and the distances calculated from \textit{Gaia} DR2 parallaxes are shown in Table \ref{Table:distance_comparison}. The typical discrepancy between the two distances is less than 1 pc; some stars have larger differences but these stars are upper limits in our analysis thus have no impact on the best-fitting relationship found in this work.
